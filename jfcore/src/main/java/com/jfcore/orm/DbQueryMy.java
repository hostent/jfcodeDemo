...
public class DbQueryMy<T> implements IDbQuery<T>{

	public DbQueryMy(String connKey, Class<T> type) {
		_connKey = connKey;
		_tEntity = new Entity<T>(type);
		trackSql = trackSql.replace("{table}", "`"+_tEntity.tableName+"`");
	}
	
	public DbQueryMy(String connKey, Class<T> type, String prefix) {
		_connKey = connKey;
...
//		columnStr = columnStr.substring(1, columnStr.length());
	...
	public long newId() {
		...
		Long res = LastIdCache.newId(_connKey, _tEntity.tableName);
		
		if(res==0)
		{
			
			Result<?> result = RedisLock.lock("newId:"+_tEntity.tableName, ()->{
				
				Long resTemp= LastIdCache.newId(_connKey, _tEntity.tableName);
				if(resTemp!=0)
				{
					return Result.succeed(resTemp);
				}
				...
				try {
					pst = BaseSqlHelp.getPreparedStatement(conn,  "select max(" + _tEntity.key + ") from "+_tEntity.tableName);
					ResultSet rs = BaseSqlHelp.getResultSet(pst);
					rs.next();
					resTemp = rs.getLong(1)+1;
					
					LastIdCache.cacheId(_connKey, _tEntity.tableName, resTemp);
